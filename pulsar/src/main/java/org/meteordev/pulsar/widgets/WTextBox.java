...
import org.meteordev.pulsar.input.CharTypedEvent;
import org.meteordev.pulsar.input.KeyEvent;
...
import org.meteordev.pulsar.theme.Selector;
import org.meteordev.pulsar.theme.Style;
import org.meteordev.pulsar.utils.CharFilters;
import org.meteordev.pulsar.utils.ICharFilter;
    ...
    protected void onKeyPressed(KeyEvent event) {
        ...
        if (control && event.key == GLFW_KEY_C) {
            if (cursor != selectionStart || cursor != selectionEnd) {
                GLFW.glfwSetClipboardString(Renderer.INSTANCE.window, text.substring(selectionStart, selectionEnd));
            }
